...
# AKILLI VERİTABANI ONARIMI
models.Base.metadata.create_all(bind=engine)
try:
    db_check = SessionLocal()
    db_check.query(models.CaseFile.anlasilan_ucret).first() # Yeni sütunu test et
    db_check.query(models.Payment).first() # Yeni tabloyu test et
    db_check.close()
except Exception as e:
    print("Sistem Uyarısı: Eski veritabanı şeması tespit edildi. A'dan Z'ye altyapı güncelleniyor...")
...
app = FastAPI(title="Merve Safa Alparslan Hukuk ERP API", version="8.0.0")
...
class ClientCaseCreate(BaseModel): 
    tc_kimlik: str; ad_soyad: str; telefon: str; eposta: str; dosya_no: str; karsi_taraf: str; tur: str; anlasilan_ucret: float
class CaseClientUpdate(BaseModel): 
    durum: str; karsi_taraf: str; anlasilan_ucret: float; telefon: str; eposta: str
...
# --- A'DAN Z'YE DÜZENLEME VE GÜNCELLEME ---
...
def update_case_all(case_id: int, req: CaseClientUpdate, db: Session = Depends(get_db)):
    ...
    # Cari hesabı güncellenen ücrete göre düzelt
...
def get_finance(db: Session = Depends(get_db)):
    ...
    # Sadece bu ayın giderleri
    giderler = db.query(models.OfficeExpense).filter(func.extract('month', models.OfficeExpense.tarih) == current_month, func.extract('year', models.OfficeExpense.tarih) == current_year).all()
    # Sadece bu ayın tahsilatları (Gelirleri)
    tahsilatlar = db.query(models.Payment).filter(func.extract('month', models.Payment.tarih) == current_month, func.extract('year', models.Payment.tarih) == current_year).all()
    
    aylik_gider = sum(g.tutar for g in giderler)
    aylik_gelir = sum(t.miktar for t in tahsilatlar)
...
def create_expense(req: ExpenseCreate, db: Session = Depends(get_db)):
    db.add(models.OfficeExpense(kalem=req.kalem, kategori=req.kategori, tutar=req.tutar)); db.commit()
...
def get_client_portal_data(client_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "finans": {"toplam": hesap.toplam_borc if hesap else 0, "odenen": hesap.odenen if hesap else 0, "kalan": (hesap.toplam_borc - hesap.odenen) if hesap else 0},
        "dosyalar": [{"id": c.id, "dosya_no": c.dosya_no, "tur": c.tur.value, "durum": c.durum, "karsi_taraf": c.karsi_taraf} for c in client.cases]
    }
...
def search_client(query: str, db: Session = Depends(get_db)): cases = db.query(models.CaseFile).join(models.Client).outerjoin(models.Document).filter((models.Client.tc_kimlik.contains(query)) | (models.CaseFile.dosya_no.contains(query)) | (models.Client.ad_soyad.ilike(f"%{query}%"))).distinct().all(); return [{"id": c.id, "dosya_no": c.dosya_no, "muvekkil": c.owner.ad_soyad, "tur": c.tur.value, "durum": c.durum, "is_closed": c.is_closed} for c in cases] if cases else {"hata": "Kayıt bulunamadı."}
